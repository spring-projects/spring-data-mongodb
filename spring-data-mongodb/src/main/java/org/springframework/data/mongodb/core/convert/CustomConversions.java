...
public class CustomConversions {
	...
	private final ConcurrentMap<ConvertiblePair, TypeValue> customReadTargetTypes;
	...
	public CustomConversions(List<?> converters) {
		...
		this.customReadTargetTypes = new ConcurrentHashMap<GenericConverter.ConvertiblePair, TypeValue>();
	...
	private Class<?> getCustomReadTarget(Class<?> sourceType, Class<?> requestedTargetType) {
		...
		TypeValue readTargetTypeValue = customReadTargetTypes.get(lookupKey);

		if (readTargetTypeValue != null) {
			return readTargetTypeValue.getType();
		}

		readTargetTypeValue = TypeValue.of(getCustomTarget(sourceType, requestedTargetType, readingPairs));
		TypeValue cacheValue = customReadTargetTypes.putIfAbsent(lookupKey, readTargetTypeValue);
		if (cacheValue != null) {
	...
	static class TypeValue {
		...
		private static final TypeValue ABSENT = new TypeValue(null);
		...
		static TypeValue of(Class<?> type) {
			return type == null ? ABSENT : new TypeValue(type);
		}
	}
}

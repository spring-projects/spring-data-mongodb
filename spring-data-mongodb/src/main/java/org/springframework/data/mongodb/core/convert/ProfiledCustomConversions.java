 ...
 */
public class ProfiledCustomConversions extends CustomConversions {
	...
	private ConcurrentHashMap<Class<?>,ConcurrentHashMap<Class<?>,CacheHolder>> cache = new ConcurrentHashMap<Class<?>, ConcurrentHashMap<Class<?>, CacheHolder>>();

	/**
	 * Creates an empty {@link CustomConversions} object.
	 */
	public ProfiledCustomConversions() {
	 ...
	 * @param converters
	 */
	public ProfiledCustomConversions(List<?> converters) {
	...
	private Class<?> getCustomReadTarget(Class<?> source, Class<?> expectedTargetType) {
		...
		Map<Class<?>, CacheHolder> map;
		CacheHolder toReturn;

		if (expectedTargetType == null) {
			expectedTargetType = Foo.class;
		}

		if ((map = cache.get(source)) == null || (toReturn = map.get(expectedTargetType)) == null) {
			Class<?> target = getCustomTarget(source, expectedTargetType, readingPairs);

			if (cache.get(source) == null) {
				cache.put(source, new ConcurrentHashMap<Class<?>, CacheHolder>());
			}

			if (target == null) {
				toReturn = new CacheHolder(null);
				cache.get(source).put(expectedTargetType, toReturn);
			} else {
				toReturn = new CacheHolder(target);
				cache.get(source).put(expectedTargetType, toReturn);
			}
		}

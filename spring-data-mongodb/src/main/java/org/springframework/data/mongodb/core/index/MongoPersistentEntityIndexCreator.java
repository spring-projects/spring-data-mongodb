	...
	void createIndex(IndexDefinitionHolder indexDefinition) {
		...
		try {
			...
			if (MongoDbErrorCodes.isDataIntegrityViolationCode(ex.getCode())) {
				...
				throw new DataIntegrityViolationException(String.format(message, indexDefinition.getPath(),
						indexDefinition.getCollection(), indexDefinition.getIndexKeys(), indexDefinition.getIndexOptions(),
						existingIndex), ex);
			}

...
public class MongoTemplate implements MongoOperations, ApplicationContextAware {
  ...
  /**
   * Constructor used for a template configuration with user credentials in the
   * form of {@link org.springframework.data.authentication.UserCredentials}
   * 
   * @param mongo
   * @param databaseName
   ...
   * {@link MongoConverter} and with a specific {@link com.mongodb.WriteConcern}
  ...
  MongoTemplate(MongoDbFactory mongoDbFactory, MongoConverter mongoConverter,
    ...
    // We always have a mapping context in the converter, whether it's a simple
    // one or not
  ...
  public CommandResult executeCommand(final DBObject command) {
    ...
    if (error != null) {
      ...
      LOGGER.warn("Command execution of " + command.toString() + " failed: "
          + error);
    }
    return result;
  }
   ...
   * (non-Javadoc)
   * 
   * @see org.springframework.data.document.mongodb.MongoOperations#execute(org.
   ...
   * 
   * @see org.springframework.data.document.mongodb.MongoOperations#execute(org.
   * springframework.data.document.mongodb.CollectionCallback, java.lang.String)
   */
  ...
  // Find methods that take a Query to express the query and that return a List
  // of objects.
   ...
   * 
   * @see
   * org.springframework.data.document.mongodb.MongoOperations#insert(java.lang
   * .Object)
   */
  ...
  /**
   * Prepare the collection before any processing is done using it. This allows
   * a convenient way to apply settings like slaveOk() etc. Can be overridden in
   * sub-classes.
   * 
   * @param collection
   */
  ...
  protected <T> void doInsertList(List<? extends T> listToSave,
    ...
    for (T o : listToSave) {
      ...
      MongoPersistentEntity<?> entity = mappingContext.getPersistentEntity(o
          .getClass());
      if (entity == null) {
  ...
  protected List<ObjectId> insertDBObjectList(String collectionName,
    ...
    if (LOGGER.isDebugEnabled()) {
      LOGGER.debug("insert list of DBObjects containing " + dbDocList.size()
          + " items");
    }
    execute(collectionName, new CollectionCallback<Void>() {
      public Void doInCollection(DBCollection collection)
        ...
        if (writeConcernToUse == null) {
          ...
          collection.insert(dbDocList
              .toArray((DBObject[]) new BasicDBObject[dbDocList.size()]),
              writeConcernToUse);
        }
        return null;
      }
   ...
   * 
   * @see
   * org.springframework.data.document.mongodb.MongoOperations#updateFirst(java
   ...
   * 
   * @see
   * org.springframework.data.document.mongodb.MongoOperations#updateMulti(java
   * .lang.String, com.mongodb.DBObject, com.mongodb.DBObject)
   */
  ...
  protected WriteResult doUpdate(final String collectionName,
    ...
    return execute(collectionName, new CollectionCallback<WriteResult>() {
      public WriteResult doInCollection(DBCollection collection)
        ...
        if (null != entityClass) {
          idProperty = getPersistentEntity(entityClass).getIdProperty()
              .getName();
        }
        for (String key : queryObj.keySet()) {
          if (idProperty.equals(key)) {
            // This is an ID field
            queryObj.put(ID,
                mongoConverter.maybeConvertObject(queryObj.get(key)));
        ...
        for (String key : updateObj.keySet()) {
          updateObj.put(key,
              mongoConverter.maybeConvertObject(updateObj.get(key)));
        }
   ...
   * 
   * @see
   * org.springframework.data.document.mongodb.MongoOperations#remove(com.mongodb
   * .DBObject)
   */
  ...
  public void remove(Object object) {
    remove(new Query(where(getIdPropertyName(object)).is(getIdValue(object))),
        object.getClass());
  }
   ...
   * 
   * @see
   * org.springframework.data.document.mongodb.MongoOperations#remove(java.lang
  ...
  /**
   * Map the results of an ad-hoc query on the default MongoDB collection to an
   * object using the template's converter
   * <p/>
   * The query document is specified as a standard DBObject and so is the fields
   * specification.
   * 
   * @param collectionName
   ...
   * instance of {@see MongoConverter}. Unless configured otherwise, an instance
   * of SimpleMongoConverter will be used.
   * <p/>
   * The query document is specified as a standard DBObject and so is the fields
   * specification.
   * <p/>
   ...
   *          the parameterized type of the returned list.
   * @param preparer
   *          allows for customization of the DBCursor used when iterating over
  ...
  protected <T> List<T> doFind(String collectionName, DBObject query,
    ...
    return executeFindMultiInternal(
        ...
        preparer, new ReadDbObjectCallback<T>(mongoConverter, entityClass),
        collectionName);
  }
  ...
  protected <T> List<T> doFind(String collectionName, DBObject query,
    ...
    MongoPersistentEntity<?> entity = mappingContext
        .getPersistentEntity(entityClass);
    return executeFindMultiInternal(
        new FindCallback(mapper.getMappedObject(query, entity), fields), null,
        new ReadDbObjectCallback<T>(readerToUse, entityClass), collectionName);
  }
  ...
  /**
   * Map the results of an ad-hoc query on the default MongoDB collection to an
   * object using the template's converter. The first document that matches the
   * query is returned and also removed from the collection in the database.
   * <p/>
   * The query document is specified as a standard DBObject and so is the fields
   * specification.
   * 
  ...
  protected <T> T doFindAndRemove(String collectionName, DBObject query,
    ...
    if (LOGGER.isDebugEnabled()) {
      LOGGER.debug("findAndRemove using query: " + query + " fields: " + fields
          + " sort: " + sort + " for class: " + entityClass
          + " in collection: " + collectionName);
    }
    MongoPersistentEntity<?> entity = mappingContext
        .getPersistentEntity(entityClass);
    return executeFindOneInternal(
        new FindAndRemoveCallback(mapper.getMappedObject(query, entity),
            fields, sort),
        new ReadDbObjectCallback<T>(readerToUse, entityClass), collectionName);
  }
  ...
  protected Object getIdValue(Object object) {
    ...
    MongoPersistentEntity<?> entity = mappingContext.getPersistentEntity(object
        .getClass());
    MongoPersistentProperty idProp = entity.getIdProperty();

    if (idProp == null) {
      throw new MappingException("No id property found for object of type "
  ...
  protected void populateIdIfNecessary(Object savedObject, Object id) {
    ...
    try {
      BeanWrapper.create(savedObject, mongoConverter.getConversionService())
          .setProperty(idProp, id);
      return;
    } catch (IllegalAccessException e) {
      throw new MappingException(e.getMessage(), e);
    } catch (InvocationTargetException e) {
   ...
   * requires reading a single object from a collection of objects. It will take
   * the following steps
   * <ol>
   ...
   *          the {@link DbObjectCallback} to transform {@link DBObject}s into
   ...
   * requires reading a collection of objects. It will take the following steps
   * <ol>
  ...
  private <T> List<T> executeFindMultiInternal(
      CollectionCallback<DBCursor> collectionCallback, CursorPreparer preparer,
      DbObjectCallback<T> objectCallback, String collectionName) {
  ...
  /**
   * Simple {@link CollectionCallback} that takes a query {@link DBObject} plus
   * an optional fields specification {@link DBObject} and executes that against
  ...
  private static class FindOneCallback implements CollectionCallback<DBObject> {
    ...
    public DBObject doInCollection(DBCollection collection)
      ...
      if (fields == null) {
        if (LOGGER.isDebugEnabled()) {
          LOGGER.debug("findOne using query: " + query + " in db.collection: "
              + collection.getFullName());
        }
        return collection.findOne(query);
      } else {
        if (LOGGER.isDebugEnabled()) {
          LOGGER.debug("findOne using query: " + query + " fields: " + fields
  ...
  private static class FindAndRemoveCallback implements
    ...
    public DBObject doInCollection(DBCollection collection)
        throws MongoException, DataAccessException {
      return collection.findAndModify(query, fields, sort, true, null, false,
          false);
    }
  }

...
Flux<ChangeStreamEvent<User>> flux = reactiveTemplate.changeStream()
    ...
    .filter(where("age").gte(38)) <1>
    .as(User.class) <2>
    .listen(); <3>
----
<1> Use an aggregation pipeline or just a query `Criteria` to filter events.
<2> The event target type the underlying document should be converted to. Leave this out to receive raw results without conversion.
